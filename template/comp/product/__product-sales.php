<div class="product-carousel --sales">
    <div class="center-wrap">
        <div class="type-carusel js-swiper__sales">
            <div class="type-carusel__head flex --align-center --just-space mb-32">
                <div class="type-carusel__title h3">Акции</div>
                <div class="type-carusel__nav flex --align-center">
                    <button class="slider-btn --prev swiper-button-prev" type="button">
                        <?php include($_SERVER['DOCUMENT_ROOT'] . '/template/img/--svg__slider-arrow.svg'); ?>
                    </button>
                    <div class="type-carusel__pagination mx-16">
                        <div class="swiper-pagination"></div>
                    </div>
                    <button class="slider-btn --next swiper-button-next" type="button">
                        <?php include($_SERVER['DOCUMENT_ROOT'] . '/template/img/--svg__slider-arrow.svg'); ?>
                    </button>
                    <div class="type-carusel__button-all-wrapper">
                        <a class="type-carusel__button-all btn --dark" href="#">Все</a>
                    </div>
                </div>
            </div>
            <div class="type-carusel__swiper">
                <div class="product-items-wrap">
                    <div class="swiper-container">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                                <div class="product-item">
                                    <div class="product-item__discount p --m">-10%</div>
                                    <div class="product-item__favor-btn btn">
                                        <span class="product-item__favor-btn-icon --svg__favor-icon"></span>
                                    </div>
                                    <div class="product-item__cover-thumb flex --align-center --just-center mr-32">
                                        <picture class="product-item__cover flex lazy">
                                            <source srcset="/template/dist/img/product-23.webp" type="image/webp">
                                            <source srcset="/template/dist/img/product-23.png" type="image/png">
                                        </picture>
                                        <div class="product-item__cover-mask" style="background-color: #F5F7F9"></div>
                                    </div>
                                    <div class="product-item__wrap flex --direction-column --align-center --just-space mt-8">
                                        <div class="product-item__head flex --align-center">
                                            <div class="product-item__details-wrap flex --just-center">
                                                <div class="product-item__articul p --s mr-12">669206</div>
                                                <div class="product-item__country p --s mr-16">Tellure Rota (Италия)
                                                </div>
                                                <div class="product-item__availability flex --align-center p --s">В
                                                    наличии
                                                </div>
                                            </div>
                                            <div class="product-item__name --p mt-8">Большегрузное поворотное с тормозом
                                                669206, полиуретан TR - полиамид 6
                                            </div>
                                            <div class="parameters-items grid --temp-col-4 mt-24">
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--1">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">80<span>x</span>60</div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--5">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">80 <span>мм</span></div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__diameter-icon">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">50 <span>мм</span></div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--7">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">50</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="product-item__bottom flex --align-center --just-space mt-20">
                                            <div class="product-item__compare-btn btn --svg__compare-icon--2"></div>
                                            <div class="product-item__price-wrap">
                                                <div class="product-item__old-price p --m">5 885 ₽</div>
                                                <div class="product-item__price">4 708 р.</div>
                                            </div>
                                            <div class="product-item__cart-btn-wrap">
                                                <div class="product-item__cart-add-btn btn --fill-1 --p">
                                                    <span class="product-item__cart-add-btn-icon --svg__cart-icon--2"></span>
                                                </div>
                                                <div class="product-item__btn-helper">
                                                    <a class="product-item__one-click-btn" data-popup="#buy-click"
                                                       href="#buy-click">Купить <br>в 1 клик</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="product-item">
                                    <div class="product-item__discount p --m">-10%</div>
                                    <div class="product-item__favor-btn btn">
                                        <span class="product-item__favor-btn-icon --svg__favor-icon"></span>
                                    </div>
                                    <div class="product-item__cover-thumb flex --align-center --just-center mr-32">
                                        <picture class="product-item__cover flex lazy">
                                            <source srcset="/template/dist/img/product-24.webp" type="image/webp">
                                            <source srcset="/template/dist/img/product-24.png" type="image/png">
                                        </picture>
                                        <div class="product-item__cover-mask" style="background-color: #E6F0F1"></div>
                                    </div>
                                    <div class="product-item__wrap flex --direction-column --align-center --just-space mt-8">
                                        <div class="product-item__head flex --align-center">
                                            <div class="product-item__details-wrap flex --just-center">
                                                <div class="product-item__articul p --s mr-12">669206</div>
                                                <div class="product-item__country p --s mr-16">Tellure Rota (Италия)
                                                </div>
                                                <div class="product-item__availability flex --align-center p --s">В
                                                    наличии
                                                </div>
                                            </div>
                                            <div class="product-item__name --p mt-8">Большегрузное поворотное с тормозом
                                                669206, полиуретан TR - полиамид 6
                                            </div>
                                            <div class="parameters-items grid --temp-col-4 mt-24">
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--1">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">80<span>x</span>60</div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--5">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">80 <span>мм</span></div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__diameter-icon">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">50 <span>мм</span></div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--7">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">50</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="product-item__bottom flex --align-center --just-space mt-20">
                                            <div class="product-item__compare-btn btn --svg__compare-icon--2"></div>
                                            <div class="product-item__price-wrap">
                                                <div class="product-item__old-price p --m">5 885 ₽</div>
                                                <div class="product-item__price">4 708 р.</div>
                                            </div>
                                            <div class="product-item__cart-btn-wrap">
                                                <div class="product-item__cart-add-btn btn --fill-1 --p">
                                                    <span class="product-item__cart-add-btn-icon --svg__cart-icon--2"></span>
                                                </div>
                                                <div class="product-item__btn-helper">
                                                    <a class="product-item__one-click-btn" data-popup="#buy-click"
                                                       href="#buy-click">Купить <br>в 1 клик</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="product-item">
                                    <div class="product-item__discount p --m">-10%</div>
                                    <div class="product-item__favor-btn btn">
                                        <span class="product-item__favor-btn-icon --svg__favor-icon"></span>
                                    </div>
                                    <div class="product-item__cover-thumb flex --align-center --just-center mr-32">
                                        <picture class="product-item__cover flex lazy">
                                            <source srcset="/template/dist/img/product-25.webp" type="image/webp">
                                            <source srcset="/template/dist/img/product-25.png" type="image/png">
                                        </picture>
                                        <div class="product-item__cover-mask" style="background-color: #F5F7F9"></div>
                                    </div>
                                    <div class="product-item__wrap flex --direction-column --align-center --just-space mt-8">
                                        <div class="product-item__head flex --align-center">
                                            <div class="product-item__details-wrap flex --just-center">
                                                <div class="product-item__articul p --s mr-12">669206</div>
                                                <div class="product-item__country p --s mr-16">Tellure Rota (Италия)
                                                </div>
                                                <div class="product-item__availability flex --align-center p --s">В
                                                    наличии
                                                </div>
                                            </div>
                                            <div class="product-item__name --p mt-8">Большегрузное поворотное с тормозом
                                                669206, полиуретан TR - полиамид 6
                                            </div>
                                            <div class="parameters-items grid --temp-col-4 mt-24">
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--1">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">80<span>x</span>60</div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--5">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">80 <span>мм</span></div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__diameter-icon">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">50 <span>мм</span></div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--7">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">50</div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="product-item__bottom flex --align-center --just-space mt-20">
                                                <div class="product-item__compare-btn btn --svg__compare-icon--2"></div>
                                                <div class="product-item__price-wrap">
                                                    <div class="product-item__old-price p --m">5 885 ₽</div>
                                                    <div class="product-item__price">4 708 р.</div>
                                                </div>
                                                <div class="product-item__cart-btn-wrap">
                                                    <div class="product-item__cart-add-btn btn --fill-1 --p">
                                                        <span class="product-item__cart-add-btn-icon --svg__cart-icon--2"></span>
                                                    </div>
                                                    <div class="product-item__btn-helper">
                                                        <a class="product-item__one-click-btn" data-popup="#buy-click"
                                                           href="#buy-click">Купить <br>в 1 клик</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="product-item">
                                    <div class="product-item__discount p --m">-10%</div>
                                    <div class="product-item__favor-btn btn">
                                        <span class="product-item__favor-btn-icon --svg__favor-icon"></span>
                                    </div>
                                    <div class="product-item__cover-thumb flex --align-center --just-center mr-32">
                                        <picture class="product-item__cover flex lazy">
                                            <source srcset="/template/dist/img/product-26.webp" type="image/webp">
                                            <source srcset="/template/dist/img/product-26.png" type="image/png">
                                        </picture>
                                        <div class="product-item__cover-mask"
                                             style="background-color: #F5F7F9"></div>
                                    </div>
                                    <div class="product-item__wrap flex --direction-column --align-center --just-space mt-8">
                                        <div class="product-item__head flex --align-center">
                                            <div class="product-item__details-wrap flex --just-center">
                                                <div class="product-item__articul p --s mr-12">669206</div>
                                                <div class="product-item__country p --s mr-16">Tellure Rota
                                                    (Италия)
                                                </div>
                                                <div class="product-item__availability flex --align-center p --s">В
                                                    наличии
                                                </div>
                                            </div>
                                            <div class="product-item__name --p mt-8">Большегрузное поворотное с
                                                тормозом 669206, полиуретан TR - полиамид 6
                                            </div>
                                            <div class="parameters-items grid --temp-col-4 mt-24">
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--1">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">80<span>x</span>60</div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--5">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">80 <span>мм</span></div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__diameter-icon">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">50 <span>мм</span></div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--7">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">50</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="product-item__bottom flex --align-center --just-space mt-20">
                                            <div class="product-item__compare-btn btn --svg__compare-icon--2"></div>
                                            <div class="product-item__price-wrap">
                                                <div class="product-item__old-price p --m">5 885 ₽</div>
                                                <div class="product-item__price">4 708 р.</div>
                                            </div>
                                            <div class="product-item__cart-btn-wrap">
                                                <div class="product-item__cart-add-btn btn --fill-1 --p">
                                                    <span class="product-item__cart-add-btn-icon --svg__cart-icon--2"></span>
                                                </div>
                                                <div class="product-item__btn-helper">
                                                    <a class="product-item__one-click-btn" data-popup="#buy-click"
                                                       href="#buy-click">Купить <br>в 1 клик</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="product-item">
                                    <div class="product-item__discount p --m">-10%</div>
                                    <div class="product-item__favor-btn btn">
                                        <span class="product-item__favor-btn-icon --svg__favor-icon"></span>
                                    </div>
                                    <div class="product-item__cover-thumb flex --align-center --just-center mr-32">
                                        <picture class="product-item__cover flex lazy">
                                            <source srcset="/template/dist/img/product-23.webp" type="image/webp">
                                            <source srcset="/template/dist/img/product-23.png" type="image/png">
                                        </picture>
                                        <div class="product-item__cover-mask"
                                             style="background-color: #F5F7F9"></div>
                                    </div>
                                    <div class="product-item__wrap flex --direction-column --align-center --just-space mt-8">
                                        <div class="product-item__head flex --align-center">
                                            <div class="product-item__details-wrap flex --just-center">
                                                <div class="product-item__articul p --s mr-12">669206</div>
                                                <div class="product-item__country p --s mr-16">Tellure Rota
                                                    (Италия)
                                                </div>
                                                <div class="product-item__availability flex --align-center p --s">В
                                                    наличии
                                                </div>
                                            </div>
                                            <div class="product-item__name --p mt-8">Большегрузное поворотное с
                                                тормозом 669206, полиуретан TR - полиамид 6
                                            </div>
                                            <div class="parameters-items grid --temp-col-4 mt-24">
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--1">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">80<span>x</span>60</div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--5">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">80 <span>мм</span></div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__diameter-icon">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">50 <span>мм</span></div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--7">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">50</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="product-item__bottom flex --align-center --just-space mt-20">
                                            <div class="product-item__compare-btn btn --svg__compare-icon--2"></div>
                                            <div class="product-item__price-wrap">
                                                <div class="product-item__old-price p --m">5 885 ₽</div>
                                                <div class="product-item__price">4 708 р.</div>
                                            </div>
                                            <div class="product-item__cart-btn-wrap">
                                                <div class="product-item__cart-add-btn btn --fill-1 --p">
                                                    <span class="product-item__cart-add-btn-icon --svg__cart-icon--2"></span>
                                                </div>
                                                <div class="product-item__btn-helper">
                                                    <a class="product-item__one-click-btn" data-popup="#buy-click"
                                                       href="#buy-click">Купить <br>в 1 клик</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="product-item">
                                    <div class="product-item__discount p --m">-10%</div>
                                    <div class="product-item__favor-btn btn">
                                        <span class="product-item__favor-btn-icon --svg__favor-icon"></span>
                                    </div>
                                    <div class="product-item__cover-thumb flex --align-center --just-center mr-32">
                                        <picture class="product-item__cover flex lazy">
                                            <source srcset="/template/dist/img/product-23.webp" type="image/webp">
                                            <source srcset="/template/dist/img/product-23.png" type="image/png">
                                        </picture>
                                        <div class="product-item__cover-mask"
                                             style="background-color: #F5F7F9"></div>
                                    </div>
                                    <div class="product-item__wrap flex --direction-column --align-center --just-space mt-8">
                                        <div class="product-item__head flex --align-center">
                                            <div class="product-item__details-wrap flex --just-center">
                                                <div class="product-item__articul p --s mr-12">669206</div>
                                                <div class="product-item__country p --s mr-16">Tellure Rota
                                                    (Италия)
                                                </div>
                                                <div class="product-item__availability flex --align-center p --s">В
                                                    наличии
                                                </div>
                                            </div>
                                            <div class="product-item__name --p mt-8">Большегрузное поворотное с
                                                тормозом 669206, полиуретан TR - полиамид 6
                                            </div>
                                            <div class="parameters-items grid --temp-col-4 mt-24">
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--1">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">80<span>x</span>60</div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--5">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">80 <span>мм</span></div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__diameter-icon">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">50 <span>мм</span></div>
                                                    </div>
                                                </div>
                                                <div class="parameter-item flex --direction-column --align-center">
                                                    <div class="parameter-item__icon --svg__parameter-icon--7">
                                                        <div class="parameter-item__icon-tooltip p --s">Допустимая
                                                            нагрузка, кг
                                                        </div>
                                                    </div>
                                                    <div class="parameter-item__desc-wrap mt-4">
                                                        <div class="parameter-item__value --p">50</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="product-item__bottom flex --align-center --just-space mt-20">
                                            <div class="product-item__compare-btn btn --svg__compare-icon--2"></div>
                                            <div class="product-item__price-wrap">
                                                <div class="product-item__old-price p --m">5 885 ₽</div>
                                                <div class="product-item__price">4 708 р.</div>
                                            </div>
                                            <div class="product-item__cart-btn-wrap">
                                                <div class="product-item__cart-add-btn btn --fill-1 --p">
                                                    <span class="product-item__cart-add-btn-icon --svg__cart-icon--2"></span>
                                                </div>
                                                <div class="product-item__btn-helper">
                                                    <a class="product-item__one-click-btn" data-popup="#buy-click"
                                                       href="#buy-click">Купить <br>в 1 клик</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
